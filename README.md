...
The complete, runnable source code for the book. It contains two things:

1. **Three installable domain packages** — `flowkit`, `heatkit`, and
   `statistical-thermodynamics` — the verified case studies of Chapters 22–24,
   each with its own `src/` layout, `pyproject.toml`, tests, and README.
2. **Per-chapter example listings** — every numbered listing in the book
   (`ch01_intro/` … `ch29_ci/`), one folder per chapter, self-contained and
...
- [Chapter & listing index](#chapter--listing-index)
  - [Foundations](#foundations)
  - [The scientific stack](#the-scientific-stack)
  - [Software engineering](#software-engineering)
  - [Numerical methods](#numerical-methods)
  - [High-performance Python](#high-performance-python)
  - [Interfaces & scientific I/O](#interfaces--scientific-io)
  - [Testing, packaging & tooling](#testing-packaging--tooling)
  - [Case studies (the installable packages)](#case-studies-the-installable-packages)
...
Every script follows PEP 8, uses type hints where they teach something, and
carries a header giving its listing number, chapter, and exact run command.
...
```
.
├── flowkit/                     # Ch 22 — Fluid Mechanics package (installable)
│   ├── pyproject.toml
│   ├── src/flowkit/             #   cavity, poisson, mesh, benchmarks
│   └── tests/
├── heatkit/                     # Ch 23 — Heat Transfer package (installable)
│   ├── pyproject.toml
│   ├── src/heatkit/             #   conduction, transient, analytical, mesh
│   └── tests/
├── statistical-thermodynamics/  # Ch 24 — Statistical Thermodynamics package
│   ├── pyproject.toml
│   ├── src/statistical_thermodynamics/   # partition functions → properties, MC
│   └── tests/
├── ch01_intro/ … ch29_ci/       # per-chapter runnable example listings
├── .github/workflows/tests.yml  # continuous integration (Ch 29)
├── requirements.txt
├── LICENSE
└── README.md
```

Chapters 25–27 and 30 (best practices, publishing to PyPI, documentation with
Sphinx, and the capstone) are discussed in the book as commands and prose rather
than standalone scripts. Chapters 28–29 ship only the reference configuration
files (`pre-commit` and GitHub Actions).
...
# Everything needed to run the per-chapter listings:
pip install -r requirements.txt

# Install a case-study package in editable mode and run its tests:
...
Each package is a complete, `pip install`-able library with its own tests and
documentation, built on the `src/` layout described in Chapter 8.

| Package | Chapter | What it does | Verified against |
|---|---|---|---|
| [`flowkit`](flowkit/) | 22 | lid-driven cavity (vorticity–streamfunction) | Ghia et al. (1982), ~0.3 % |
| [`heatkit`](heatkit/) | 23 | unified planar/cylindrical/spherical conduction | exact analytical profiles |
| [`statistical-thermodynamics`](statistical-thermodynamics/) | 24 | partition functions → thermodynamic properties, Monte Carlo | experiment + compute-two-ways |

---

## Requirements

**Python 3.12 or newer.** `pip install -r requirements.txt` covers the
per-chapter listings; the packages declare their own dependencies in each
`pyproject.toml`.

| Package        | Minimum  | Used by                              | Purpose                                   |
|----------------|----------|--------------------------------------|-------------------------------------------|
| **numpy**      | 1.26     | all chapters + packages              | arrays, vectorization, linear algebra     |
| **scipy**      | 1.11     | Ch 5, 11–14, 23; heatkit             | integration, solvers, special functions   |
...
| pytest         | 7.4      | Ch 20 + every package's tests        | test runner                               |
...
**Tests (Chapter 20).** The example test suite runs under pytest:
...
The listings below are grouped by theme. Every entry links to its source file.

### Foundations

*The scientific mindset, modern Python idioms, and the data structures numerical
work is built on.*
...
*Floating-point reality, linear systems, ODEs and PDEs, integration,
optimization, and interpolation — each verified against theory.*
...
### Case studies (the installable packages)

*Complete solvers that put the whole book to work — each checked against an
analytical solution or a published benchmark. These are also the three
installable packages above; the `chNN_*` folders keep the single-file versions
shown in the book.*

| Chapter | File / package | What it does |
|---|---|---|
| 22 | [cavity.py](ch22_fluid_mechanics/cavity.py) · [`flowkit`](flowkit/) | lid-driven cavity, verified vs Ghia et al. (1982) |
| 22 | [cavity_verify.py](ch22_fluid_mechanics/cavity_verify.py) | compact one-file verification of the cavity result |
| 23 | [conduction.py](ch23_heat_transfer/conduction.py) · [`heatkit`](heatkit/) | unified planar / cylindrical / spherical conduction |
| 24 | [partition_and_error.py](ch24_statistical_thermo/partition_and_error.py) · [`statistical-thermodynamics`](statistical-thermodynamics/) | partition function two ways + Monte-Carlo blocking error |
...
Released under the **MIT License** — see [`LICENSE`](LICENSE) and each package.
You are free to use, modify, and share the code, including in teaching and
research, with attribution. The accompanying book text is © I. L. Ferreira.
